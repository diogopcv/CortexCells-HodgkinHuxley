...
vvaz = -80; %potencial vazamento
cm = 4*area; %capacitancia especifica
gnamax = 380*area; %maxima condutancia especifica sodio
gkmax = 79*area; %maxima condutancia especifica potassio
gkmmax = 1.2*area;
gvaz = 0.2*area; %condutancia vazamento
cond1 = (2.174*pi*diama^2)/(4*compa);
% dendritos ---------------------------------------------------------------
compd = 5e-3;
diamd = 4e-4;
aread = pi*diamd*compd;
cond2 = (2.174*pi*diamd^2)/(4*compd);
gvazd = 0.2*aread;
cmd = 4*aread;
%-------------------------------------------------------------------------
...
f1 = (-(Ik + Ina + Ikm + Ivaz + I2s + I3s)+in)/cm ;
